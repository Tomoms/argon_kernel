...
#define USE_STATE_NOTIFIER

#include <linux/cpu.h>
#ifdef USE_STATE_NOTIFIER
...
#if (defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_POWERSUSPEND) && !defined(DISABLE_POWER_MANAGEMENT)) || defined(USE_STATE_NOTIFIER)
...
#if defined(USE_STATE_NOTIFIER) && !defined(CONFIG_POWERSUSPEND)
static struct notifier_block zzmoove_lcd_notif;
#endif /* defined(USE_STATE_NOTIFIER)... */
...
#if defined(CONFIG_HAS_EARLYSUSPEND) && !defined(USE_STATE_NOTIFIER)
static void __cpuinit powersave_early_suspend(struct early_suspend *handler)
#elif defined(CONFIG_POWERSUSPEND) && !defined(USE_STATE_NOTIFIER) || defined(CONFIG_POWERSUSPEND) && defined(USE_STATE_NOTIFIER)
static void __cpuinit powersave_suspend(struct power_suspend *handler)
#elif defined(USE_STATE_NOTIFIER)
...
#if defined(CONFIG_HAS_EARLYSUSPEND) && !defined(USE_STATE_NOTIFIER) && !defined(DISABLE_POWER_MANAGEMENT)
static struct early_suspend __refdata _powersave_early_suspend = {
  .suspend = powersave_early_suspend,
  .resume = powersave_late_resume,
  .level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN,
};
#elif defined(CONFIG_POWERSUSPEND) && defined(USE_STATE_NOTIFIER) && !defined (DISABLE_POWER_MANAGEMENT) || defined(CONFIG_POWERSUSPEND) && !defined(USE_STATE_NOTIFIER) && !defined (DISABLE_POWER_MANAGEMENT)
...
static int __init cpufreq_gov_dbs_init(void)						// ZZ: idle exit time handling
{
	...
	zzmoove_lcd_notif.notifier_call = zzmoove_state_notifier_callback;
	if (state_register_client(&zzmoove_lcd_notif) != 0) {
...
#if (defined(USE_STATE_NOTIFIER) && !defined(CONFIG_POWERSUSPEND))
	state_unregister_client(&zzmoove_lcd_notif);
#endif /* (defined(USE_STATE_NOTIFIER)... */
}
